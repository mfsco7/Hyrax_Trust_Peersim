...
public class BitTorrent implements EDProtocol {
    ...
    /**
     * The status of the local peer. 0 means that the current peer is a
     * leecher, 1 a seeder.
     */
    ...
    public BitTorrent(String prefix) { // Used for the tracker's protocol
        ...
        peersetSize = (int) Configuration.getInt(prefix + "." +
                PAR_PEERSET_SIZE);
        numberOfDuplicatedRequests = (int) Configuration.getInt(prefix + "" +
                "." + PAR_DUP_REQ);
    ...
    /**
     * Gets the number of neighbors currently stored in the cache of the
     * local node.
     *
     ...
     *
     * @param node  reference to the local node for which the event is going
     *              to be processed
     * @param pid   peersim.BitTorrent's protocol id
    ...
    public void processEvent(Node node, int pid, Object event) {
        ...
        switch (((SimpleEvent) event).getType()) {
            ...
            case KEEP_ALIVE: // 1
            {
                ...
                //System.out.println("process, keep_alive: sender is "+sender
                ...
                if (e != null) { //if I know the sender
                    ...
                    System.err.println("despite it should never happen, it "
                            + "happened");
                    ev = new BitfieldMsg(BITFIELD, true, false, node, status,
                            nPieces);
                    latency = ((Transport) node.getProtocol(tid)).getLatency
                            (node, sender);
            ...
            case CHOKE: // 2, CHOKE message.
            {
                ...
                //System.out.println("process, choke: sender is "+sender
                // .getID()+", local is "+node.getID());
                Element e = search(sender.getID());
                if (e != null) { //if I know the sender
                    cache[e.peer].isAlive();
            ...
            case UNCHOKE: // 3, UNCHOKE message.
            {
                ...
                Node sender = ((SimpleMsg) event).getSender();
                //System.out.println("process, unchoke: sender is "+sender
                // .getID()+", local is "+node.getID());
                Element e = search(sender.getID());
                if (e != null) { // If I know the sender
                    ...
                    while (block != -2) { //while still available request to
                        ...
                        if (block < 0) { // No more block to request for the
                            ...
                            for (int j = 0; j < swarmSize; j++) {// send the
                                ...
                                if (alive(cache[j].node) && swarm[j][piece]
                                        == 1) {

                                    ev = new IntMsg(INTERESTED, node,
                                            lastInterested);
                                    latency = ((Transport) node.getProtocol
                                            (tid)).getLatency(node, cache[j]
                                            .node);
                                    EDSimulator.add(latency, ev, cache[j]
                                            .node, pid);
                                    cache[j].justSent();
                                }
            ...
            case NOT_INTERESTED: // 5, NOT_INTERESTED message.
            {
                numInterestedPeers--;
                Node sender = ((IntMsg) event).getSender();
                //System.out.println("process, not_interested: sender is
            ...
            case HAVE: // 6, HAVE message.
            {
                ...
                Node sender = ((IntMsg) event).getSender();
                //System.out.println("process, have: sender is "+sender.getID
                // ()+", local is "+node.getID());
            ...
            case BITFIELD: // 7, BITFIELD message
            {
                ...
                if (!((BitfieldMsg) event).isRequest && !((BitfieldMsg)
                        event).ack) {
                    Element e = search(sender.getID());
                    if (e == null) // if is a response with nack that follows
                        // a request
                        nBitfieldSent--;
                    // otherwise is a response with ack that follows a duplicate
                    // insertion attempt
                    //System.out.println("process, bitfield_resp_nack: sender
                    // is "+sender.getID()+", local is "+node.getID());
                    return;
                }
                /*peersim.Request with NACK*/
                if (((BitfieldMsg) event).isRequest && !((BitfieldMsg) event)
                        .ack) {
                    //System.out.println("process, bitfield_req_nack: sender
                    // is "+sender.getID()+", local is "+node.getID());
                    if (alive(sender)) {
                        Element e = search(sender.getID());
                        ev = new BitfieldMsg(BITFIELD, false, true, node,
                                status, nPieces); //response with ack
                        latency = ((Transport) node.getProtocol(tid))
                                .getLatency(node, sender);
                        EDSimulator.add(latency, ev, sender, pid);
                        cache[e.peer].justSent();
                ...
                if (!((BitfieldMsg) event).isRequest && ((BitfieldMsg) event)
                        .ack) {
                    nBitfieldSent--;
                    //System.out.println("process, bitfield_resp_ack: sender
                    // is "+sender.getID()+", local is "+node.getID());
                    if (alive(sender)) {
                            ...
                            if (nNodes == 10 && !lock) { // I begin to
                                ...
                                for (int i = 0; i < swarmSize; i++) {// send
                                    ...
                                    if (alive(cache[i].node) &&
                                        ...
                                        latency = ((Transport) node
                                                .getProtocol(tid)).getLatency
                                                (node, cache[i].node);
                                        EDSimulator.add(latency, ev, cache[i]
                                                .node, pid);
                                        cache[i].justSent();
                                    }
                                }
                ...
                if (((BitfieldMsg) event).isRequest && ((BitfieldMsg) event)
                    ...
                    if (alive(sender)) {
                        if (addNeighbor(sender)) {
                            ...
                            boolean isSeeder = true;
                            for (int i = 0; i < nPieces; i++) {
                                rarestPieceSet[i] += fileStatus[i]; // I
                                // update the rarestPieceSet with the pieces
                                // of the new node
                                isSeeder = isSeeder && (fileStatus[i] == 1);
                                // I check if the new node is a seeder
                            }
                            e.isSeeder = isSeeder;
                            ev = new BitfieldMsg(BITFIELD, false, true, node,
                                    status, nPieces); //response with ack
                            ...
                            if ((e = search(sender.getID())) != null) { //
                                ...
                                cache[e.peer].isAlive();
                                ev = new BitfieldMsg(BITFIELD, false, true,
                                        node, status, nPieces); //response
                                // with ack
                                latency = ((Transport) node.getProtocol(tid))
                                        .getLatency(node, sender);
                                EDSimulator.add(latency, ev, sender, pid);
                                cache[e.peer].justSent();
                            } else { // Was not be possible add the sender
                                // (nBitfield+nNodes > swarmSize)
                                ev = new BitfieldMsg(BITFIELD, false, false,
                                        node, status, nPieces); //response
                                // with nack
                                latency = ((Transport) node.getProtocol(tid))
                                        .getLatency(node, sender);
            ...
            case REQUEST: // 8, REQUEST message.
            {
                ...
                while (!requestToServe.empty() && nPiecesUp < 10) {
                    ...
                    if (e != null && alive(req.sender)) {
                        ev = new IntMsg(PIECE, node, req.id, requestTime);
                        nPiecesUp++;
                        e.valueUP++;
                        senderP = ((BitTorrent) req.sender.getProtocol(pid));
                        senderP.nPiecesDown++;
                        remoteRate = senderP.maxBandwidth / (senderP
                                .nPiecesUp + senderP.nPiecesDown);
						...
						is completed.
						This prevent that, when the receiver death occurres,
						my value nPiecesUp
						doesn't decrease.*/
            ...
            case PIECE: // 9, PIECE message.
            {
                ...
                if (peerStatus == 1)// To save CPU cycles
                    return;
                //System.out.println("process, piece: sender is "+sender
                // .getID()+", local is "+node.getID());
                Element e = search(sender.getID());
				...
				/* If the block has not been already downloaded and it
				belongs to
					the current downloading piece.*/
                if (piece == currentPiece && decode(pieceStatus[block], 0) !=
                        piece) {
                ...
                for (int i = 0; i < swarmSize; i++) {
                    if (alive(cache[i].node) && unchokedBy[i] == true &&
                            swarm[i][decode(block, 0)] == 1 && cache[i].node
                            != sender) {
                        latency = ((Transport) node.getProtocol(tid))
                ...
                if (status[currentPiece] == 16) { // if piece completed, I
                    ...
                    for (int i = 0; i < swarmSize; i++) { // I send the HAVE
                        // for the piece
                        if (alive(cache[i].node)) {
                            latency = ((Transport) node.getProtocol(tid))
                                    .getLatency(node, cache[i].node);
                        ...
                        if (!alive(cache[i].node)) {
                            //System.out.println("piece3 rm neigh "+ cache[i]
                            // .node.getID() );
                    ...
                    for (int i = 0; i < swarmSize; i++) { // I send the
                        // NOT_INTERESTED to which peer I sent an INTERESTED
                        ...
                        if (!alive(cache[i].node)) {
                            //System.out.println("piece4 rm neigh "+ cache[i]
                            // .node.getID() );
                    ...
                    if (nPieceCompleted == nPieces) {
                        System.out.println("FILE COMPLETED for peer " + node
                                .getID());
                        this.peerStatus = 1;
                    }
                    ...
                    while (m > 0) { // I process the queue
                        ...
                        if (s == null) // if the node that sent the block in
                            // the queue is dead
                            continue;
                        if (p == currentPiece && decode(pieceStatus[b], 0) !=
                                p) {
                            pieceStatus[b] = temp.id;
                            status[p]++;
                            removeRequest(temp.id);
                            requestNextBlocks(node, pid, s.peer);
                        } else { // The piece not currently desired will be
                            // moved to the tail
                            if (p != currentPiece) // If not a duplicate
                                // block but belongs to another piece
            ...
            case CANCEL: {
                //TODO when requester send cancel messages remove from
                // interactions
                Node sender = ((IntMsg) event).getSender();
                int value = ((IntMsg) event).getInt();
            ...
            case PEERSET: // PEERSET message
            {
                ...
                for (int i = 0; i < peersetSize; i++) {
                    if (n[i] != null && alive(n[i].node) && search(n[i].node
                            .getID()) == null && nNodes + nBitfieldSent <
                            swarmSize - 2) {
                        ev = new BitfieldMsg(BITFIELD, true, true, node,
                                status, nPieces);
                        latency = ((Transport) node.getProtocol(tid))
                                .getLatency(node, n[i].node);
            ...
            case TRACKER: // TRACKER message
            {
                ...
                if (nNodes <= peersetSize) {
                    for (int i = 0; i < nMaxNodes + maxGrowth; i++) {
                        if (cache[i].node != null && cache[i].node.getID() !=
                                sender.getID()) {
                            tmp[k] = cache[i];
                            k++;
                        }
                ...
                while (j < peersetSize) {
                    ...
                    for (int z = 0; z < j; z++) {
                        if (cache[i].node == null || tmp[z].node.getID() ==
                                cache[i].node.getID() || cache[i].node.getID
                                () == sender.getID()) {
                            z = 0;
            ...
            case CHOKE_TIME: //Every 10 secs.
            {
                ...
                for (int i = 0; i < swarmSize; i++) { // I perform the chokes
                    // and the unchokes
                    if ((i == luckies[0] || i == luckies[1] || i ==
                            luckies[2]) && alive(cache[i].node) && cache[i]
                            .status != 2) { //the unchokes
                        cache[i].status = 1;
                        ...
                        if (alive(cache[i].node) && (cache[i].status == 1 ||
                                cache[i].status == 2)) {
                            cache[i].status = 0;
            ...
            case OPTUNCHK_TIME: {
                ...
                int lucky = CommonState.r.nextInt(nNodes);
                while (cache[byPeer[lucky].peer].status == 1)// until the
                    // lucky peer is already unchoked
                    lucky = CommonState.r.nextInt(nNodes);
                if (!alive(cache[byPeer[lucky].peer].node)) return;
                cache[byPeer[lucky].peer].status = 1;
                Object msg = new SimpleMsg(UNCHOKE, node);
                latency = ((Transport) node.getProtocol(tid)).getLatency
                        (node, cache[byPeer[lucky].peer].node);
                EDSimulator.add(latency, msg, cache[byPeer[lucky].peer].node,
                        pid);
            ...
            case ANTISNUB_TIME: {
                if (this.peerStatus == 1) // I'm a seeder, I don't update the
                    // event
                    return;
                //System.out.println("process, antisnub_time");
                for (int i = 0; i < nNodes; i++) {
                    if (byPeer[i].valueDOWN > 0 && (byPeer[i].valueDOWN -
                            byPeer[i].head60) == 0) {// No blocks downloaded
                        // in 1 min
                ...
                for (int i = 0; i < swarmSize; i++) {
					...
					minutes ago*/
                        ...
                        if (cache[i].lastSeen < (now - 121000) && cache[i]
                                ...
                                121000)) {
                            System.out.println("process, checkalive_time, rm " +
                                    "neigh " + cache[i].node.getID());
                            if (cache[i].node.getIndex() != -1) {
                                System.out.println("This should never happen:" +
                                        " I remove a node that is not " +
                                        "effectively died");
                            }
                            removeNeighbor(cache[i].node);
                            processNeighborListSize(node, pid);
    ...
    /**
     * Given a piece index and a block index it encodes them in an unique
     * integer value.
     *
    ...
    /**
     * Returns either the piece or the block that contained in the
     * <tt>value</tt> depending
     * on <tt>part</tt>: 0 means the piece value, 1 the block value.
     *
     * @param value the ID of the block to decode.
     * @param part  the information to extract from <tt>value</tt>. 0 means
     *              the piece index, 1 the block index.
    ...
    /**
     * Used by {@link NodeInitializer#choosePieces(int, BitTorrent) peersim
     ...
     * @return <tt>false</tt> if the neighbor is already present in the cache
     * (this can happen when the peer requests a
     * new peer set to the tracker an there is still this neighbor within) or
    ...
    public boolean addNeighbor(BitNode neighbor) {
        ...
        if (this.tracker == null) { // I'm in the tracker's peersim.bittorrent
            // .BitTorrent protocol
            for (int i = 0; i < nMaxNodes + maxGrowth; i++) {
                if (cache[i].node == null) {
                    ...
                    //System.err.println("i: " + i +" nMaxNodes: " +
                    // nMaxNodes);
                    return true;
                }
            }
            ...
            if ((nNodes + nBitfieldSent) < swarmSize) {
                //System.out.println("I'm the node " + this.thisNodeID + ",
                // trying to add node "+neighbor.getID());
                for (int i = 0; i < swarmSize; i++) {
                    if (cache[i].node == null) {
                        cache[i].node = neighbor;
    ...
    private void removeRequest(int id) {
        int i = 4;
        for (; i >= 0; i--) {
            if (pendingRequest[i] == id) break;
        }
        for (; i >= 0; i--) {
            if (i == 0) pendingRequest[i] = -1;
            else pendingRequest[i] = pendingRequest[i - 1];
        }
    }

    /**
     * Requests new block until the {@link #pendingRequest} is full to the
     * sender of the just received piece.
     * It calls {@link #getNewBlock(Node, int)} to implement the <i>strict
     * priority</i> strategy.
     *
     * @param node   the local node
     * @param pid    the peersim.BitTorrent protocol id
        ...
        while (block != -2) {
            if (unchokedBy[sender] == true && alive(cache[sender].node) &&
                ...
                if (!alive(cache[sender].node) && cache[sender].node != null) {
                    System.out.println("piece2 rm neigh " + cache[sender]
                            .node.getID());
    ...
    /**
     * It returns the id of the next block to request. Sends
     * <tt>INTERESTED</tt> if the new
        ...
        if (block < 0) { // No more block to request for the current piece
            ...
            for (int j = 0; j < swarmSize; j++) {// send the interested
                ...
                if (alive(cache[j].node) && swarm[j][newPiece] == 1) {
                    long latency = ((Transport) node.getProtocol(tid))
                            .getLatency(node, cache[j].node);
                    EDSimulator.add(latency, ev, cache[j].node, pid);
                    cache[j].justSent();
                }
                if (!alive(cache[j].node)) {
                    //System.out.println("piece1 rm neigh "+ cache[j].node
                    // .getID() );
    ...
    private int getBlock() {
        int i = 4;
        while (i >= 0 && pendingRequest[i] != -1) { // i is the first empty
        ...
        int j;
        //The queue is not empty & last requested block belongs to
        // lastInterested piece
     ...
     * -1 is returned.
     * @see "Documentation about the peersim.BitTorrent module"
     */
    private int getPiece() {
        int piece = -1;
        if (nPieceCompleted < 4) { //Uses random first piece
            piece = CommonState.r.nextInt(nPieces);
            while (status[piece] == 16 || piece == currentPiece) // until the
                // piece is owned
                piece = CommonState.r.nextInt(nPieces);
            return piece;
            ...
            int rarestPieces[] = new int[nPieces - j]; // the pieces with the
            // less number of occurences\
            rarestPieces[0] = j;
            int nValues = 1; // number of pieces less distributed in the network
            for (int i = j + 1; i < nPieces; i++) { // Finds the rarest piece
                // not owned
                if (rarestPieceSet[i] < rarestPieceSet[rarestPieces[0]] &&
                        status[i] == 0) { // if strictly less than the
                    // current one
                    rarestPieces[0] = i;
                    nValues = 1;
                }
            ...
            piece = CommonState.r.nextInt(nValues); // one of the less owned
            // pieces
    ...
    private int partition(int left, int right) {
        ...
        int j = right;
        while (true) {
            while (greater(byBandwidth[++i], byBandwidth[right]))      //
     ...
     * @param y the second <tt>peersim.Element</tt> to compare
    ...
    private boolean greater(Element x, Element y) {
        ...
        // if peerStatus==1 (the local node is a seeder)
            ...
            if ((x.valueUP - x.head20) > (y.valueUP - y.head20)) return true;
            else return false;
        }
    }
     ...
     * @return the {@link Element} in {@link #byPeer} which represents the
     * node with the
     * given ID.
     */
    public Element search(long ID) {
        ...
        while (low <= high) {
            if (byPeer[p] == null || byPeer[p].ID > ID) high = p - 1;
            else {
                if (byPeer[p].ID < ID)  //Wasteful second comparison forced
                    // by syntax limitations.
                    low = p + 1;
                else return byPeer[p];
            }
 ...
 * {@link peersim.bittorrent.BitTorrent#byPeer} and {@link peersim.bittorrent
 * .BitTorrent#byBandwidth}.
 */
class Element {
    /**
    ...
    /**
     * Index position of the node in the {@link peersim.bittorrent
     * .BitTorrent#cache} array.
     */
    public int peer = -1;
    /**
    ...
    /**
     * Makes a deep copy of the peersim.Element to <tt>destination</tt>
     *
     * @param destination peersim.Element instance where to make
     *                    the copy
     */
...
class Neighbor {
     ...
     * 0 means CHOKED<br/>
     * 1 means UNCHOKED<br/>
     * 2 means SNUBBED_BY. If this value is set and the node is to be
     * unchocked,
...
/**
 * Class type of the queues's items in {@link peersim.bittorrent.BitTorrent
 * #incomingPieces}
 * and {@link peersim.bittorrent.BitTorrent#requestToServe}.
 */
class Queue {
    int maxSize;
    int head = 0;
    ...
    public boolean contains(int value) {
        ...
        for (int i = head; i < head + dim; i++) {
            if (queue[i % maxSize].id == value) return true;
        }
        return false;
    }
    ...
    public boolean remove(Node sender, int value) {
        if (empty()) return false;
        for (int i = head; i < head + dim; i++) {
            if (queue[i % maxSize].id == value && queue[i % maxSize].sender
                    == sender) {
